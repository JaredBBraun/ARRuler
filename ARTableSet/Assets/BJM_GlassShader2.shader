...
Shader "ShaderTest/BJM_GlassShader2" {
    Properties {
    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
				...
				float2 blurUV00 = float2(sceneUVs.x - 0.0015, sceneUVs.y - 0.0015);
				float2 blurUV01 = float2(sceneUVs.x, sceneUVs.y - 0.0015);
				float2 blurUV02 = float2(sceneUVs.x + 0.0015, sceneUVs.y - 0.0015);
				float2 blurUV10 = float2(sceneUVs.x - 0.0015, sceneUVs.y);
				float2 blurUV12 = float2(sceneUVs.x + 0.0015, sceneUVs.y);
				float2 blurUV20 = float2(sceneUVs.x - 0.0015, sceneUVs.y + 0.0015);
				float2 blurUV21 = float2(sceneUVs.x, sceneUVs.y + 0.0015);
				float2 blurUV22 = float2(sceneUVs.x + 0.0015, sceneUVs.y + 0.0015);

				float4 sceneColor00 = tex2D(_GrabTexture, blurUV00);
				float4 sceneColor01 = tex2D(_GrabTexture, blurUV01);
				float4 sceneColor02 = tex2D(_GrabTexture, blurUV02);
				float4 sceneColor10 = tex2D(_GrabTexture, blurUV10);
				float4 sceneColor11 = tex2D(_GrabTexture, sceneUVs);
				float4 sceneColor12 = tex2D(_GrabTexture, blurUV12);
				float4 sceneColor20 = tex2D(_GrabTexture, blurUV20);
				float4 sceneColor21 = tex2D(_GrabTexture, blurUV21);
				float4 sceneColor22 = tex2D(_GrabTexture, blurUV22);

				float4 sceneColor = (sceneColor00 + sceneColor01 + sceneColor02 + sceneColor10 + sceneColor11 + sceneColor12 + sceneColor20 + sceneColor21 + sceneColor22) / 9.0;
////// Lighting:
////// Emissive:
